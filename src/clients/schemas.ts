...
const WorkflowTestMetricsSchema = z.object({
  average_test_count: z.number().int().min(0),
  most_failed_tests: z.array(
    z.object({
      ...
      p95_duration: z.number().nullable(),
  ...
  most_failed_tests_extra: z.number().int().min(0),
  ...
  slowest_tests_extra: z.number().int().min(0),
  total_test_runs: z.number().int().min(0),
  test_runs: z.array(
    z.object({
      pipeline_number: z.number().int().min(0),
      workflow_id: z.any(),
      success_rate: z.number(),
      test_counts: z.object({}).passthrough(),
    }),
  ),
});
