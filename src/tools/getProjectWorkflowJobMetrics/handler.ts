...
import { JobMetrics } from '../../clients/schemas.js';

export const projectWorkflowJobMetrics: ToolCallback<{
  params: typeof projectWorkflowJobMetricsInputSchema;
}> = async (args) => {
  const {
    workspaceRoot,
    gitRemoteURL,
  ...
  try {
    ...
    if (jobMetrics.length === 0) {
      return {
        content: [
          {
            type: 'text',
            text: `No job metrics found for workflow "${workflowName}" with the specified filters.`,
          },
        ],
      };
    }
    ...
    const formattedMetrics = jobMetrics.map(formatJobMetrics);
    ...
    return {
      content: [
        {
          type: 'text',
          text: `Successfully retrieved metrics for ${formattedMetrics.length} jobs in workflow "${workflowName}" for project ${projectSlug}.`,
        },
        {
          type: 'text',
    ...
    return mcpErrorOutput(
      `Error retrieving job metrics: ${(error as Error).message}`,
    );
  }
};

/**
 * Formats job metrics for display
 */
function formatJobMetrics(metric: JobMetrics) {
  const {
    name,
    metrics: {
      total_runs,
